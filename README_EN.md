...
| `bitcask_txn:transaction/2,3`, `read/2,3`, `write/3`, `delete/2`, `abort/1` | **Isolated transactions** (unreleased): pessimistic 2PL point locks + deadlock detection + automatic restart, Mnesia-style; `{atomic,R} \| {aborted,Why}`; options `retries`/`timeout`/`lock_wait_timeout`/`sync`/`index_fun`; `read/3` with `write` takes the write lock up front for read-modify-write; Fun must be side-effect free (may run more than once) |
| `graphdb:transaction/2,3`, `put_edge_txn/4,5`, `del_edge_txn/4,5`, `edge_txn/4,5`, `degree_txn/3`, `put_vertex_txn/3`, `get_vertex_txn/2` | Transactional graph writes: an edge's five keys + degree counters commit in one locked batch, so concurrent edge inserts keep exact counts; do not mix with direct `put_edge` on the same graph |
...
| `ROADMAP_EN.md` / `ROADMAP.md` | **Roadmap**: 6.4.0 / 6.3.2 / 6.3.1 / 6.2.2 / 6.2.1 / 6.1.0 / 6.0.0 / 5.1.0 / 5.0.0 / 4.0.0 / 3.1.0 / 3.0.0 shipped + 2.1.1 shipped (P5–P15) + 2.2.0 plan (libcask extraction / V7+ vector optimization) (EN/中) |
