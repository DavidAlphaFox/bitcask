...
对 Erlang 调用方**无 API 变更**（C API 与 C++ 公开头零改动，`SOVERSION` 保持 6，
盘上格式不动、无迁移），重编即得。值得记一笔的三件：

- **撕裂尾部（torn tail）覆盖** — 写入偏移锚定到最后一个完整 record，崩溃 /
  掉电后尾部半条 record 不再被当成有效数据。这是本次升级里对生产最有价值的一条。
- **符号链接 workaround 删除** — 上游全部路径改用 `PROJECT_SOURCE_DIR`，被
  `add_subdirectory` 嵌入时不再解析到下游仓库树里。这件事**只能在上游修**：
  CMake 进子目录作用域会重设 `CMAKE_SOURCE_DIR`，下游覆盖无效。
- **锁文件加了第二行** — 进程实例令牌，防 PID 复用误判 stale lock（POSIX 恒为
  `0`）。刻意加成第二行而不是扩第一行，于是两个解析器（只读首行）新旧双向兼容。

Windows 移植（MSVC 原生 x64）本身对本仓库不构成交付面——本仓库的 NIF 不在
Windows 上构建；带入的是它顺手做的那批 Linux 也吃得到的加固（io seam、运行期
SIMD 探测、GCC 14 告警清理）。
...
`range` 的「索引不可用」从一个 `no_index` 拆成两个：

- `{error, no_index}` — 索引在**本句柄上本就不建**（只读 / `merge_only` 打开一个
  从未建过 OKI 的目录）。读写方式重开即自动建。
- `{error, index_rebuild_failed}`（新）— 可写 open 时**试建而败**（IO / 环境
  问题，细节在 log 里）。修完环境重开重试。

> 拆的理由是**补救方式不同**，不是为了精确而精确。合成一个码时，调用方唯一
> 合理的处理是「没索引 → 回落到全表扫」——这对两种成因都说得通，但第二种其实
> 是环境坏了、值得告警，而合成码把这个信号抹掉了。
>
> ⚠️ 两者都没有被压成空结果 `[]`：`index_rebuild_failed` 意味着**数据在、索引
> 不在**，返回空是实打实的数据丢失假象。OKI 只是派生缓存——这两种情况下
> `get` / `put` / `fold` 全部照常，只有 range 不可用。

上游 6.1.0 是 MINOR、纯增量：新枚举值**追加在 `CaskError` 尾部**，既有值不动
...
HTTP 档（openai / anthropic）也实现了原生批量：一次请求带数组，`max_batch` 切块。
⚠️ 响应**按 `index` 字段归位不按返回顺序 zip**——按顺序 zip 会把向量配到别的文档上，
不报错、维度也对。顺带把两个 HTTP provider 完全重复的请求/解析收进
`bitcask_embedder_util`，解析做成纯函数并补了 14 条不打网络的单测（此前只有一个
默认跳过的手动用例）。
