...
| V3.6 | search_hybrid RRF + NIF/Erlang 接口:SearchLayer::search_hybrid(两路 K'=max(k×4,64),score=Σ1/(60+rank),rank 从 1 起;**平局 → ord 小者在前**;单路空退化为另一路 RRF 重打分,双空/无向量配置/维度不符 → kInvalidOption)。NIF 跨界 = f32 LE 二进制(<< <<X:32/float-little>> >>):put doc map 新增 vector 键(坏尺寸 → badarg 不落盘)、cask_search_vector/4(Ef=0 → max(K,64))、cask_search_hybrid/4、open {vector_dim,N}/{vector_metric,cosine\|l2\|dot}。bitcask_embedder behaviour(embed/1 → {ok, f32LE} ;可选 dim/0)+ bitcask_embedder_openai(OpenAI 兼容 /v1/embeddings,httpc + OTP≥27 内置 json,32K→32768 字节保守截断 + UTF-8 尾回退);eunit 用确定性 mock(bitcask_embedder_mock),真实端点用例默认 skip(BITCASK_EMBEDDER_LIVE=1)。测试:C++ +3(RRF 精确浮点平局真值/单路退化/边界)+ eunit +6(embed→put→hybrid 全链路/坏 binary/重开)。门禁:plain/TSan/ASan ctest 381/381(TSan 零报告,detect_deadlocks=1)+ ldd 无 tsan + eunit 50/50 | ✅ |
| V3.7 | 基准定稿:hnsw_bench.cpp(Insert 10k/100k、Search {10k,100k}×{ef64,ef256})+ BM_Cask_SearchHybrid,28 条聚合入 baseline.json。**查询红线 ✅**(100k/ef64 483µs < 1ms;hybrid 端到端 270µs);**插入红线 100k 档 ❌**(936/s < 2k/s;10k 档 3.0k/s ✅)——内存受限归因+残差路线记 hnsw-design §6,前台写不受阻(IndexPool 异步) | ✅(插入残差如实记录) |
| V3.8 | 距离内核 4 路累加器展开(破 FMA 依赖链)+ 候选向量软件预取(256B 预取遍,取数与计算重叠)。实测 384d:查询 100k/ef64 **483→322µs(−33%)**、10k 档 −19%、hybrid 270→250µs;插入 100k **936→1221/s(+30%)**、10k 3.0→3.4k/s——**插入红线仍未达**(残余在 efC 候选搜索访存量,根治 V4 量化)。baseline.json 已换新数;门禁:plain/ASan/TSan 381/381 + 召回门不变 + eunit 50/50 | ✅ |
...
| **P3 向量落盘 int8 量化** | data file 向量 f32（2560 维=10KB/doc）→ 落盘 int8（4× 磁盘+读 I/O）。设计见 `doc/vector-ondisk-quant-design-zh.md`。**P3a ✅** codec int8 编解码（码字 `[Dim][SchemeVer:u8][scale:f32][int8×Dim]`，复用 `vec::int8`）+ `doc_vector_f32` + fixture。**P3b ✅** open `{vector_quantized,true}` 接线 + meta 持久化（offset[9]）+ 重开一致校验（mismatch→mode_mismatch）+ put 落盘 int8 + get/recovery dequant（`GetResultView` 加 owned dequant 缓冲）；顺带修了 Erlang open 遇裸 atom 故障（mode_mismatch）case_clause 崩溃的潜在 bug。**P3c**（真实语料召回测量定是否作默认；HNSW 直接吃 codes 的 CPU 微优化）为 follow-on。 | 中高 / 高 | 🚧 P3a·P3b ✅ / P3c 待 |
