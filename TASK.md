...
| **S9.28** | search_phrase 查询词序不可靠 | `search_layer.cpp` SearchLayer::search_phrase | 做 S8.7 时发现的预存隐患：`search_phrase` 用 `for (auto& [term,_] : analyzer_->analyze(query))` 遍历 **unordered_map** 取 terms，**词序不保证**。而短语匹配（inverted::search_phrase_impl 的有序匹配）依赖查询词序——map 顺序随实现/哈希变化，可能让多词短语（≥3 词）匹配行为不确定（漏召或错召）。S8.7 的 `search_near` 已规避：用 `analyze_with_positions` 按 position 排序还原词序。修法：search_phrase 同样改用 analyze_with_positions 排序取 terms。**未做**（独立预存问题，改后需补多词短语词序回归测试） | 🟡 中 | ☐ |
