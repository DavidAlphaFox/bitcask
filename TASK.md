...
| **M7-9** | 回归：`rebar3 eunit` **110/110**（19 本地嵌入 + 26 embedder 进程/池；NIF 未构建时优雅跳过 0.05s）+ `rebar3 xref` 干净 + 端到端（`open` 配 embedder → `put` 自动 embed → `search_vector`/`search_hybrid`）。⚠️ **CUDA 编译与真实 GPU 运行路径未实测**（开发机无驱动无 toolkit；Vulkan 只是编出来、没有卡可跑）。池的机制已用 mock provider 测透（稳定名字、直接派发、忙闲换人、worker 被 kill 不拖垮整池、`open` 收池名、app env 端到端），但**多卡上的真实 GPU 行为没跑过**——有卡的机器上跑一遍才算真验证。 | ✅ |
