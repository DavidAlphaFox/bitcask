...
| **S9.21** | min_token_length 等 analyzer 参数的 NIF 暴露 | `atoms.*` `nif_helpers.cpp` `bitcask.erl` | 核实 S9.8 时发现：NIF 选项解析只透传 analyzer type / dict_path / enable_stop_words，**连 min_n/max_n 都没暴露**，故 min_token_length 也无法从 Erlang 配置。C++ 层已支持，缺 Erlang 侧透传（加 atom + parse_2tuple_option 分支 + CASK_PASSTHROUGH）。属既有边界，**未做** | 低 | ☐ |
...
| **S9.20** | 高亮重复片段 | `highlighter.cpp` select_best_fragments | 修 S9.19 时观察到：同一匹配会生成多个重叠片段（`world` 高亮出现 3 次相同 snippet）。`select_best_fragments` 的窗口选取未对已覆盖区间去重。非坐标 bug，独立的展示质量问题，**未做** | 低 | ☐ |
