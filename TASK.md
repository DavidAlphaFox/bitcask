...
| **M7-7j** | `instances => auto`：取**ggml 枚举到的 ∩ 装得下模型的**卡（后者复用加载前的粗检，顺带跳过别人占着显存的卡）。⚠️ 查证后推翻了自己先前的反对理由——"auto 等于让库默认占满整机"站不住：两个后端各自都有设备可见性 env var（`GGML_CUDA_DEVICES` / `GGML_VK_VISIBLE_DEVICES`，外加 CUDA 自己的 `CUDA_VISIBLE_DEVICES`），可见性本来就是运维侧的标准手段。⚠️ **失败策略不对称**：显式列表全都必须起来（写下卡号是意图声明），auto 尽力而为（child start 返回 `ignore`，supervisor 记 undefined 并继续——**实测确认过这条机制**）。⚠️ 配套 `status/1` 报 requested/started/missing：8 卡只起 1 个时业务拿到 1/8 吞吐而一切看着正常，这是唯一线索。无可用卡 → 退化成一个不绑卡的 CPU instance，不报错。新增可选 provider 回调 `auto_instances/1`，没实现的 provider（HTTP 档）明确拒绝 auto。 | ✅ |
| **M7-7k** | `per_gpu => K`：同卡开 K 个 context。⚠️ 一个 context 同时只服务一次前向，多开确实多几路并发，但抢同一批 SM、收益不一定线性，显存按份数涨。**无实测数据 → 显式选项、不进默认路径。** 与"同一张卡出现在两个 instance 里"的校验区分开：后者是配置错，前者是明确要求的复制。 | ✅ |
| **M7-9** | 回归：`rebar3 eunit` **138/138**（25 本地嵌入 + 39 embedder 进程/池/批量 + 14 HTTP 解析；NIF 未构建时优雅跳过 0.05s）+ `rebar3 xref` 干净 + `rebar3 dialyzer` **零警告**（`plt_extra_apps: [inets]`——用它而非把 inets 加进 `applications`，运行期依赖不变） + 端到端（`open` 配 embedder → `put` 自动 embed → `search_vector`/`search_hybrid`）。⚠️ **CUDA 编译与真实 GPU 运行路径未实测**（开发机无驱动无 toolkit；Vulkan 只是编出来、没有卡可跑）。池的机制已用 mock provider 测透（稳定名字、直接派发、忙闲换人、worker 被 kill 不拖垮整池、`open` 收池名、app env 端到端），但**多卡上的真实 GPU 行为没跑过**——有卡的机器上跑一遍才算真验证。 | ✅ |
