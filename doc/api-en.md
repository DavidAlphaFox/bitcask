...
| `{embedder, ServerRef}` | **Recommended for stateful providers.** A [`bitcask_embedder_server`](../src/bitcask_embedder_server.erl) process. Same behaviour, but the provider state lives in that process: **shared across casks**, lifecycle tied to the process (released in `terminate/2`). ⚠️ Local models (`bitcask_embedder_llama`) should use this — `{Provider,Cfg}` loads one copy of the weights per open, and `bitcask:close/1` does **not** release the embedder. | `ServerRef = pid() \| registered name \| {global,_} \| {via,M,N}`. Not started → `{error, {embedder_not_running, Ref}}`; if it dies at runtime `put` gets `{error, {embedder_not_running,_}}` and the **caller is not killed**. See [`doc/local-embedding-zh.md`](local-embedding-zh.md). |
