...
### 4.3 配置 / 接线 / 持久化
- `bitcask:open` 选项 `{vector_inmem_int8, true}` → NIF → `CaskOptions.vector_inmem_int8`
...
### 4.4 快照（BCVS）
- BCVS 快照已持 int8（load 时写 qcodes）。int8-only 模式快照**只存 codes+scale**（不存 f32），
  load 直接建 int8-only 图。需 BCVS 版本位标记 inmem_int8 或由 meta 决定。
