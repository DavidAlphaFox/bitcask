...
| 写法 | 含义 | 起不来怎么办 |
|---|---|---|
| `[0, 1, 2, 3]` | 4 个 instance，各占 1 张卡 | **全都必须起来**，否则 application 起不来 |
| `[[0,1], [2,3]]` | 2 个 instance，各跨 2 张卡（单卡装不下时） | 同上 |
| `auto` | 由 provider 探测：**枚举到的、且装得下模型的**卡，一卡一个 | **尽力而为**：起不来的跳过，一个都没起来才算失败 |

`per_gpu => K`（默认 1）在此基础上把每个 instance 复制 K 份——同一张卡开 K 个
...
- 「枚举到的」——这个进程能看见哪几张卡本来就是**运维侧的标准手段**：
  `GGML_CUDA_DEVICES` / `CUDA_VISIBLE_DEVICES` / `GGML_VK_VISIBLE_DEVICES` /
  容器的设备透传。我们不另造一套，也不假装比运维更懂该用哪几张。
- 「装得下的」——复用加载前那次粗检（GGUF 大小 vs 该卡可用显存）。顺带解决了
  "别人正占着显存的卡"：自动跳过，而不是起到一半 OOM。
  ⚠️ 仍然只是粗检（不含 compute buffer / KV cache），所以是**筛选**不是保证。

一张可用的都没有 → 退化成**一个不绑卡的 instance**（纯 CPU）。⚠️ 不报错是有意的：
`auto` 的语义是"有什么用什么"，没卡时它仍该可用，否则一台机器没卡就让整个
application 起不来。

**失败策略不对称，这是有意的。** 写下卡号是一种**意图声明**——你说了要用 2 号卡，
它起不来就是配置与现实不符，该死给你看。而 `auto` 本来就是"有什么用什么"，一张卡
忙着不该拖垮整个 application。

> ⚠️ **"尽力而为"必须配"说得出来"**：8 张卡只起来 1 个时，业务拿到的是 1/8 的
> 吞吐而一切看起来正常。跳过的每一个都会记 warning，并且可以问：
>
> ```erlang
...
> ```
>
> ⚠️ provider 没实现 `auto_instances/1`（比如 HTTP 档，它根本没有卡的概念）时
> `auto` 会被**明确拒绝**（`{error, {instances_auto_unsupported, _}}`），不会悄悄
> 退化成"起一个"——那会让 `instances => auto` 看起来生效了。
