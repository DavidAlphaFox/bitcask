...
%% -------------------------------------------------------------------
%% =========================================================================
%% Module bitcask_legacy: contains the original bitcask implementation for
%% the legacy C NIF (priv/bitcask.so) and the fine-grained C++ NIF
%% (priv/bitcask_cpp.so). The cask_cpp path lives in bitcask.erl directly.
%%
%% bitcask.erl is the user-visible facade. In `legacy` / `cpp` modes, every
%% public bitcask:* call routes through bitcask_legacy:* defined below.
%% =========================================================================
-module(bitcask_legacy).
...
open(Dirname, Opts) ->
    ...
    %% NIF dispatch modes:
    %%   cask_cpp — new C++ NIF, coarse-grained cask_* API (DEFAULT as of M4)
    %%   cpp      — new C++ NIF, fine-grained keydir/file calls (M2.5)
    %%   legacy   — original C NIF, fine-grained keydir/file calls (opt-in)
    %%
    %% The default lives in `default_nif_mode/0`. To roll back to legacy
    %% globally, set the application env: `application:set_env(bitcask,
    %% default_nif_mode, legacy)`.
...
%% M4.1 default-mode policy.
%%
%% In production builds, the default is `cask_cpp` so a fresh `bitcask:open(D)`
%% transparently runs through the new C++ NIF + Cask facade — no opt-in required.
%%
%% Under -DTEST (rebar3 eunit), the default is pinned to `legacy` because the
%% pre-existing 80+ tests are white-box: they reach into `#bc_state` internals
%% via get_state/1, which only exists on the legacy path. Honouring those
%% guarantees keeps the historical regression suite intact, while the
%% explicit `bitcask_cpp_*_tests` modules continue to opt into the new NIF
%% paths via {nifs, cpp} / {nifs, cask_cpp}.
%%
%% Override with `application:set_env(bitcask, default_nif_mode, M)` for
%% either rollback (M = legacy in prod) or testing the new default.
default_nif_mode() ->
...
return_cask_open(Dirname, Opts) ->
    %% Translate the subset of legacy bitcask:open options that map cleanly
    %% to the coarse-grained cask_open NIF. Unknown opts are silently
    %% dropped (legacy did the same).
...
list_data_files_race_test() ->
    ...
    ReadFiles = lists:usort(bitcask_legacy:readable_files(Dir)),
    meck:unload(),
...
fold_corrupt_file_test2() ->
    ...
    CreateFun = fun(KVList) ->
                       ...
                       close(B),
                       bitcask_legacy:readable_files(TestDir)
               end,
...
update_tombstones_test() ->
    ...
    ok = bitcask:close(B2),
    Files = bitcask_legacy:readable_files(Dir),
    Fds = [begin
    ...
    TombCount = bitcask_legacy:subfold(CountF, Fds, 0),
